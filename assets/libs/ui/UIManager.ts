...
export class UIManager {
    ...
    private getOrCreateUI(uiId: number, processCallback: ProgressCallback | null, completeCallback: (uiView: UIView | null) => void, uiArgs: any): void {
        ...
        // 找到UI配置
        const uiConf = this.UIConf[uiId];
        if (null == uiConf) {
            logger.logView(`getOrCreateUI ${uiId} faile, prefab conf not found!`);
            completeCallback(null);
            return;
        }
        const {prefab: uiPath, bundle} = uiConf;
        resLoader.load(bundle, uiPath, processCallback, (err, prefab) => {
            // 检查加载资源错误
            if (err) {
                logger.logView(`getOrCreateUI loadRes ${uiId} faile, path: ${uiPath} error: ${err}`);
                completeCallback(null);
                return;
            }
            // 检查实例化错误
            ...
            if (null == uiNode) {
                logger.logView(`getOrCreateUI instantiate ${uiId} faile, path: ${uiPath}`);
                completeCallback(null);
                prefab.decRef();
                return;
            }
            ...
            if (null == uiView) {
                logger.logView(`getOrCreateUI getComponent ${uiId} faile, path: ${uiPath}`);
                uiNode.destroy();
                completeCallback(null);
                prefab.decRef();
                return;
